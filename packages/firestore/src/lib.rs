...
pub fn sync_collection<T>(
    param: &T::ParamForPath,
    mut on_change: impl FnMut(Vec<T>) + 'static,
    ...
    T: FireStoreResource,
{
    ...
    let callback = move |json: String| match serde_json::from_str(json.as_str()) {
        Ok(t) => on_change(t),
        Err(e) => {
    ...
    T: FireStoreResource,
{
    ...
    let on_error = move || on_error.borrow_mut()();
    crate::js_bridge::sync_document_json(
        (T::path(param) + "/" + document_id).as_str(),
        callback,
        on_error,
    )
}
